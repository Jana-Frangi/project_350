"""
Πthon Arena - Server (static obstacles, leaderboard, rematch)
...
# Static obstacles (walls + extra blocks)
OBSTACLES = WALLS + [
    [5,5], [5,6], [6,5],
    [15,10], [15,11],
    [20,20], [21,20], [20,21],
...
class IthonArenaServer:
    def __init__(self, port):
        ...
        self.player_stats = {}   # username -> {wins, longest, pies, powerups}
        ...
        try:
            ...
            with self.lock:
                ...
                if username not in self.player_stats:
                    self.player_stats[username] = {"wins":0, "longest":0, "pies":0, "powerups":0}
    ...
    def _handle_message(self, username, msg):
        ...
        if mtype == "move" and username in self.current_players:
            ...
            sorted_stats = sorted(self.player_stats.items(), key=lambda x: x[1].get("wins",0), reverse=True)
            self._send(self.players[username]["conn"], {"type": "leaderboard", "data": dict(sorted_stats[:10])})
    ...
    def _tick(self):
        ...
        for pu in self.game_state["powerups"][:]:
            for uname, snake in self.game_state["snakes"].items():
                if not snake["alive"]:
                    continue
                if snake["body"][0] == pu["pos"]:
                    ...
                    if uname in self.player_stats:
                        self.player_stats[uname]["powerups"] += 1
        ...
        for username, snake in list(self.game_state["snakes"].items()):
            ...
            for _ in range(speed):
                ...
                if new_head in self.game_state["obstacles"]:
                    if snake["shield"] <= 0:
                ...
                for pie in self.game_state["pies"][:]:
                    if new_head == pie["pos"]:
                        ...
                        self._spawn_pie()
                        ate = True
                        if username in self.player_stats:
                            self.player_stats[username]["pies"] += 1
                        break
                if not ate:
                    snake["body"].pop()
                # Update longest snake length
                length = len(snake["body"])
                if username in self.player_stats and length > self.player_stats[username]["longest"]:
                    self.player_stats[username]["longest"] = length
                if snake["health"] <= 0:
                    snake["alive"] = False
                    self._check_game_over()
                if speed == 2:
    ...
    def _reset_snake(self, username):
        while True:
            ...
            if [x,y] not in self.game_state["obstacles"]:
                break
    ...
    def _finish_game(self, winner, reason="game_over"):
        ...
        if winner and winner in self.player_stats:
    ...
    def _init_snake(self, username, color):
        ...
        while True:
            x = random.randint(3, GRID_W-4)
            y = random.randint(3, GRID_H-4)
            if [x,y] not in taken and [x,y] not in self.game_state["obstacles"]:
                break
